...
import { faChartArea, faDollarSign, faUserPlus } from "@fortawesome/free-solid-svg-icons";
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const getDashboardData = async () => {
      try {
        console.log("dashboard: " + authContext.authState.token);
      ...
      {dashboardData ? (
        <>
          ...
          <div className="w-full mt-4">
            <Card>{dashboardData && <DashboardChart salesData={dashboardData.graphData} />}</Card>
          </div>
        </>
      ) : (
        <p>Loading...</p>
